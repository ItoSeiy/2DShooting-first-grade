...
public class BossNormalAttack04 : MonoBehaviour
{
    ...
    IEnumerator Attack()
    {
        while (true)
        {
            ...
            Vector3 localAngle = _muzzles[0].localEulerAngles;
            ...
            for (float rotation = MINIMUM_ROT_RANGE + localAngle.z; rotation < MAXIMUM_ROT_RANGE + localAngle.z; rotation += _rotationInterval)
            {
                ///マズルを回転する///
                //Vector3 localAngle = _muzzles[0].localEulerAngles;// ローカル座標を基準に取得
                //角度を設定
                localAngle.z = rotation;
