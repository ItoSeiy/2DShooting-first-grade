...
public class SuperAttack05 : MonoBehaviour
{
    ...
    IEnumerator A()
    {
        ...
        while (true)
        {
            ...
            Vector3 leftLocalAngle = _muzzles[0].localEulerAngles;// ローカル座標を基準に取得
            leftLocalAngle.z -= 10.0f;// 角度を設定
            _muzzles[0].localEulerAngles = leftLocalAngle;//回転する
            //弾を発射（仮でBombにしてます）
